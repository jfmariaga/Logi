        ...
        <div class="col-md-12 mt-3">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    @foreach ($allPermissions->whereIn('name', ['ver programación', 'crear programación', 'editar programación', 'eliminar programación']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach
                ...
                <div class="col-md-4">
                    ...
                    @foreach ($allPermissions->whereIn('name', ['ver capacitaciones', 'crear capacitaciones', 'editar capacitaciones', 'eliminar capacitaciones']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach

                    <h6 class="text-uppercase text-primary mt-3">Cursos</h6>
                    @foreach ($allPermissions->whereIn('name', ['ver materiales', 'ver preguntas', 'ver asignaciones', 'ver resultados', 'ver mis cursos']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach

                    <h6 class="text-uppercase text-primary mt-3">Información de Interés</h6>
                    @foreach ($allPermissions->whereIn('name', ['ver información de interes', 'crear información de interes', 'editar información de interes', 'eliminar información de interes']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach

                    <h6 class="text-uppercase text-primary mt-3">Usuarios</h6>
                    @foreach ($allPermissions->whereIn('name', ['ver usuarios', 'crear usuarios', 'editar usuarios', 'eliminar usuarios']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach

                    <h6 class="text-uppercase text-primary mt-3">EPP y Dotación</h6>
                    @foreach ($allPermissions->whereIn('name', ['EPP y Dotación', 'Productos', 'Inventario', 'Entregas', 'Mis entregas']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach
                </div>
                ...
                <div class="col-md-4">
                    ...
                    @foreach ($allPermissions->whereIn('name', ['ver sedes', 'crear sedes', 'editar sedes', 'eliminar sedes']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach

                    <h6 class="text-uppercase text-primary mt-3">Gestión Documental</h6>
                    @foreach ($allPermissions->whereIn('name', ['ver gestión documental', 'crear gestión documental', 'editar gestión documental', 'eliminar gestión documental']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach

                    <h6 class="text-uppercase text-primary mt-3">Roles</h6>
                    @foreach ($allPermissions->whereIn('name', ['ver roles', 'crear roles', 'editar roles', 'eliminar roles']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach

                    <h6 class="text-uppercase text-primary mt-3">Debida Diligencia</h6>
                    @foreach ($allPermissions->whereIn('name', ['ver listado', 'ver formularios', 'aprobar formularios', 'modificar notificaciones']) as $p)
                        @include('admin.roles.checkbox', ['p' => $p])
                    @endforeach
