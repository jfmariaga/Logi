<div x-data="data_calendario">
    ...
    <div class="calendario-container">
                        ...
                        <div class="col-5 col-md-4">
                            <div class="m-1">
                                ...
                                <select class="form-control select2" id="filtro_tipo">
                                    <option @if( $filtro_tipo == '0' ) selected @endif value="0">Sedes</option>
                                    <option @if( $filtro_tipo == '1' ) selected @endif value="1">Usuarios</option>
                                </select>
                            </div>
                        </div>
        ...
        <div class="mes-selector mt-3">
            ...
            <h5><b class="mes-actual">{{ $mesesEnEspañol[date('F', mktime(0, 0, 0, $mesActual, 1))] }} {{ $anoActual }}</b></h5>
                ...
                @foreach ($diasDelMes as $semana)
                    <tr class="dias">
                        @foreach ($semana as $dia)
                            ...
                            @endphp
                            <td class="dia-mes{{ $dia == '' ? ' disabled hide_responsive' : '' }} {{ $esDiaYMesActual ? 'dia-actual' : '' }}">
                                <div class="dia-numero d-flex">
                                    {{ $dia ? date('d', strtotime( $fecha )) : '' }}
                                    <span class="show_responsive">&nbsp;-&nbsp;{{ $diasEnEspañol[ date( 'D', strtotime( $fecha ) ) ] ?? '' }} </span>
                                </div>

                                <div onclick="showXFecha( '{{ $fecha }}' )" data-toggle="modal" data-target="#showEventosXFecha" class="content_dia_calendario scroll_x w-100">
                                    <div>
