    ...
    function __resetTable( id, type_order = 'desc' ){
        $( id ).DataTable().destroy(); // destruimos la tabla
        $( id ).DataTable({ // le asignamos nuevamente las propiedades de datatables
            ...
            order:[[0, type_order]], // si se necesita cambiar como parámetro y se asigna esta por defecto
            // dom: 'Bfrtip',
            dom: `<'row head_tabla'<'col-sm-12 text-right'B><'col-md-6 col-sm-12 mostrar_jota'l><'col-md-6 col-sm-12'f>r>t<'row mt-2 footer_tabla'<'col-md-6 col-sm-12'i><'col-md-6 col-sm-12'p>>"`,
            buttons: [
                {
                  extend: 'excelHtml5',
    ...
    function __resetTableNoPaginate( id ){
        $( id ).DataTable().destroy(); // destruimos la tabla
        $( id ).DataTable({ // le asignamos nuevamente las propiedades de datatables
            language: {
                "decimal": "",
                "emptyTable": "No se han cargado archivos",
                "info": "Mostrando _START_ a _END_ de _TOTAL_ Entradas",
                "infoEmpty": "Mostrando 0 to 0 of 0 Entradas",
                "infoFiltered": "(Filtrado de _MAX_ total entradas)",
                "infoPostFix": "",
                ...
                "processing": "Procesando...",
                "search": "Buscar:",
                "zeroRecords": "No se han cargado archivos",
                "paginate": {
                    "first": "Primero",
                    "last": "Último",
                    "next": "Siguiente",
    ...
    function limiparSelect2( id ){
        $(`#${id}`).val('').trigger('change');
    }

    // saca una copia completa rompiendo toda referencia
    function __duplicar( data ){
